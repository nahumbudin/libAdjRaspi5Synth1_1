...
serialPortData_t *control_box_rx_alsa_channel_data = NULL;
...
void *ControlBoxClientAlsaOutput::alsaOutThread(void *threadid)
{
	...
	while (alsa_out_thread_is_running)
	{
		...
		control_box_rx_alsa_channel_data = SerialPort::alsa_control_box_control_serial_port_rx_queue.dequeue(0, &timeout, &non_blocking);
		// Get all new data
		count = 0;
		last_count = 0;
		while (control_box_rx_alsa_channel_data != NULL)
		{
			// Collect all available data
			for (count; count < (last_count + control_box_rx_alsa_channel_data->mssg_len) &&
						count < sizeof(control_box_rx_alsa_channel_data->message) &&
						count < sizeof(bytes);
				 count++)
			{
				bytes[count] = control_box_rx_alsa_channel_data->message[count - last_count];
			}
			last_count = count;
			delete control_box_rx_alsa_channel_data;
			control_box_rx_alsa_channel_data = SerialPort::alsa_control_box_control_serial_port_rx_queue.dequeue(0, &timeout, &non_blocking);
