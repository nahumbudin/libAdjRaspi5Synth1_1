...
*   @return void
*/
void AudioVoiceFloat::set_magnitude(float mag)
{
	magnitude = mag;
...
/**
*   @brief  Return the voice output magnitude (volume)
*   @param  none
*   @return the voice output magnitude (volume) 0.0 to 1.0
*/
float AudioVoiceFloat::get_magnitude() 
{ 
	return magnitude; 
}
...
void AudioVoiceFloat::update()
{
	...
	for (i = 0; i < audio_block_size; i++) 
	{
		...
		// magnitude - Note scaled volume (see kbd)
		
		block_out1->data[i] = dsp_voice->get_next_output_value_ch_1() * magnitude;
		block_out2->data[i] = dsp_voice->get_next_output_value_ch_2() * magnitude; 
	}
